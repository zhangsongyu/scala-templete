...
trait Json4sSupport {
  ...
  /**
    ...
    */
  implicit def marshaller[A <: Any](implicit serialization: Serialization,
                                       formats: Formats,
                                       shouldWritePretty: ShouldWritePretty =
                                       ShouldWritePretty.False): ToEntityMarshaller[A] =
    shouldWritePretty match {
      case ShouldWritePretty.False =>
        jsonStringMarshaller.compose(x=>serialization.write[AnyRef](x.asInstanceOf[AnyRef]))
      case ShouldWritePretty.True =>
        jsonStringMarshaller.compose(x=>serialization.writePretty[AnyRef](x.asInstanceOf[AnyRef]))
    }
}
